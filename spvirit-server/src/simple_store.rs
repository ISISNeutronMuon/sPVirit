...
impl PvStore for SimplePvStore {
    ...
    async fn get_snapshot(&self, name: &str) -> Option<NtPayload> {
        let pvs = self.pvs.read().await;
        pvs.get(name).map(|e| e.record.to_ntpayload())
    }

    async fn get_descriptor(&self, name: &str) -> Option<StructureDesc> {
        let pvs = self.pvs.read().await;
        pvs.get(name)
    ...
    match &mut record.data {
        ...
        RecordData::NtStructure { nt } => {
            ...
            if let Some(next) = decoded_to_nt_structure(value) {
                ...
                debug!(
                    "PUT to NtStructure '{}' rejected: not a structure value",
                    record.name
                );
            }
        }
...
fn decoded_to_nt_field(value: &DecodedValue) -> Option<NtField> {
    match value {
        ...
        DecodedValue::Array(items) => {
            Some(NtField::ScalarArray(decoded_array_to_scalar_array(items)))
        }
...
fn decoded_array_to_scalar_array(items: &[DecodedValue]) -> ScalarArrayValue {
    macro_rules! collect_arm {
        ($variant:ident, $ty:ty, $extract:expr) => {{ ScalarArrayValue::$variant(items.iter().filter_map($extract).collect::<Vec<$ty>>()) }};
    }
    match items.first() {

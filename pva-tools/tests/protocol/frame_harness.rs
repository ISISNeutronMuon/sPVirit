...
use pva_tools::pva_client::client::{build_client_validation, encode_create_channel_request};
...
impl TestServer {
    ...
    pub fn spawn_with_args(extra_args: &[&str]) -> Result<Self, String> {
        ...
        let server_bin = workspace_bin("pva_server");
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| format!("spawn pva_server: {}", e))?;
        ...
        while start.elapsed() < ready_timeout {
            if let Ok(Some(status)) = child.try_wait() {
                return Err(format!("pva_server exited early: {}", status));
            }
        ...
        if !ready {
            return Err(format!(
                "pva_server not ready on {} within {:?}",
                addr, ready_timeout
            ));
        }

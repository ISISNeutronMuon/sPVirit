...
fn run_rust_read_tools(server: Option<&str>, pv: &str) {
    let pvget_bin = workspace_bin("pvget");
    let mut pvget = rust_tool_command(&pvget_bin, server, pv);
    let pvget_output = run_command(&mut pvget, "rust pvget")
    ...
    let pvinfo_bin = workspace_bin("pvinfo");
    let mut pvinfo = rust_tool_command(&pvinfo_bin, server, pv);
    let pvinfo_output = run_command(&mut pvinfo, "rust pvinfo")
...
fn run_rust_write_restore(server: Option<&str>, pv: &str) {
    ...
    let pvput_bin = workspace_bin("pvput");

    let mut read_before = rust_tool_command(&workspace_bin("pvget"), server, pv);

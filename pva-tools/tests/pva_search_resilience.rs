...
use pva_tools::pva_client::search::{discover_servers, search_pv, SearchTarget};
use pva_tools::pva_client::types::PvGetError;
...
async fn search_pv_skips_bad_target_and_succeeds_on_next() {
    let (mut server, _tcp_port, udp_port) = match start_test_server() {
        Some(parts) => parts,
        None => {
            eprintln!("Skipping test: cannot spawn local pva_server");
            return;
        }
    };

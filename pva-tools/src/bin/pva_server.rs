...
use pva_tools::pva_server::db::load_db;
use pva_tools::pva_server::state::{ConnState, MonitorState, MonitorSub};
use pva_tools::pva_server::types::{
    LinkExpr, NtPayload, NtScalar, OutputMode, RecordData, RecordInstance, ScalarArrayValue,
    ScalarValue, ScanMode,
};

use pva_codec::epics_decode::{PvaHeader, PvaPacket, PvaPacketCommand};
use pva_codec::pva_encode::encode_control_message;
use pva_codec::pva_encode::{
...
use pva_codec::pvd_decode::{DecodedValue, PvdDecoder};
use pva_codec::pvd_decode::{FieldDesc, FieldType, StructureDesc, TypeCode};
use pva_codec::pvd_encode::{encode_size_pvd, nt_payload_desc};
...
async fn handle_connection(
    ...
    loop {
        ...
        if cmd_code == 1 {
            ...
            let validation = pva_codec::epics_decode::PvaConnectionValidationPayload::new(
                payload_slice,
                is_be,
                false,
            );
            if let Some(val) = validation {
                ...
                let resp = pva_codec::pva_encode::encode_connection_validated(version, is_be);
...
async fn handle_get_field_request(
    ...
    conn_id: u64,
    payload: pva_codec::epics_decode::PvaGetFieldPayload,
    version: u8,
    is_be: bool,
) {
    if payload.is_server {
        ...
        if let Some(pv_name) = conn_state.sid_to_pv.get(&sid) {
            if let Some(nt) = get_nt_snapshot(state, pv_name).await {
                ...
                let desc = if let Some(field_path) = sub {
                    use pva_codec::pvd_decode::extract_subfield_desc;
...
async fn handle_server_rpc(
    ...
    let payload = NtPayload::ScalarArray(pva_tools::pva_server::types::NtScalarArray::from_value(
...
fn virtual_pvlist_nt(entries: Vec<String>) -> NtPayload {
    NtPayload::ScalarArray(pva_tools::pva_server::types::NtScalarArray::from_value(
...
fn apply_scalar_array_put(
    nt: &mut pva_tools::pva_server::types::NtScalarArray,
    nord: &mut usize,
    value: &DecodedValue,
) -> bool {
...
fn apply_table_put(nt: &mut pva_tools::pva_server::types::NtTable, value: &DecodedValue) -> bool {
    let DecodedValue::Structure(fields) = value else {
        return false;
    };
    let mut changed = false;
...
fn apply_ndarray_put(nt: &mut pva_tools::pva_server::types::NtNdArray, value: &DecodedValue) -> bool {
    let DecodedValue::Structure(fields) = value else {
        return false;
    };
    let mut changed = false;
    for (name, field_value) in fields {
        match name.as_str() {
            ...
            "dimension" => {
                if let DecodedValue::Array(items) = field_value {
                    let dims: Vec<pva_tools::pva_server::types::NdDimension> = items
                        .iter()
                        .filter_map(|item| {
                            if let DecodedValue::Structure(fs) = item {
                                Some(pva_tools::pva_server::types::NdDimension {
            ...
            "attribute" => {
                if let DecodedValue::Array(items) = field_value {
                    let attrs: Vec<pva_tools::pva_server::types::NtAttribute> = items
                        .iter()
                        .filter_map(|item| {
                            if let DecodedValue::Structure(fs) = item {
                                ...
                                Some(pva_tools::pva_server::types::NtAttribute {
                                    name: attr_name,
                                    value: attr_value,
                                    descriptor,
                                    source_type,
...
fn apply_value_update(nt: &mut NtScalar, val: &DecodedValue, compute_alarms: bool) -> bool {
    ...
    match &mut nt.value {
        pva_tools::pva_server::types::ScalarValue::Bool(current) => {
        ...
        pva_tools::pva_server::types::ScalarValue::I32(current) => {
        ...
        pva_tools::pva_server::types::ScalarValue::F64(current) => {
        ...
        pva_tools::pva_server::types::ScalarValue::Str(current) => {
...
fn decode_nt_alarm(val: &DecodedValue) -> Option<pva_tools::pva_server::types::NtAlarm> {
    let DecodedValue::Structure(fields) = val else {
        return None;
    };
    ...
    Some(pva_tools::pva_server::types::NtAlarm { severity, status, message })
}

fn decode_nt_timestamp(val: &DecodedValue) -> Option<pva_tools::pva_server::types::NtTimeStamp> {
    let DecodedValue::Structure(fields) = val else {
        return None;
    };
    ...
    Some(pva_tools::pva_server::types::NtTimeStamp { seconds_past_epoch: seconds, nanoseconds: nanos, user_tag })
}

fn decode_nt_display(val: &DecodedValue) -> Option<pva_tools::pva_server::types::NtDisplay> {
    let DecodedValue::Structure(fields) = val else {
        return None;
    };
    ...
    Some(pva_tools::pva_server::types::NtDisplay { limit_low, limit_high, description, units, precision })
}

fn decode_put_body(
    body: &[u8],
    desc: &pva_codec::pvd_decode::StructureDesc,
    is_be: bool,
) -> Option<DecodedValue> {
...
mod tests {
    use super::*;
    use pva_codec::pvd_encode::{encode_nt_scalar_bitset_parts, nt_scalar_desc};

    #[test]
    fn decode_put_body_accepts_status_prefix() {
        let nt = NtScalar::from_value(pva_tools::pva_server::types::ScalarValue::F64(2.5));
    ...
    fn decode_put_body_accepts_bitset_without_struct_bit() {
        let nt = NtScalar::from_value(pva_tools::pva_server::types::ScalarValue::F64(0.0));
        let desc = nt_scalar_desc(&nt.value);
        let mut body = Vec::new();
    ...
    fn apply_value_update_refreshes_alarm_from_limits() {
        let mut nt = NtScalar::from_value(pva_tools::pva_server::types::ScalarValue::F64(0.0))
